...
void Camera::SetSize(int w, int h, float n, float f)
{
  ...
  const float e = 1.0f / std::tan(GH::FOV * GH::PI / 360.0f);
  const float a = float(height) / float(width);
  const float d = near_ - far_;
  ...
  projection.m[8] = 0.0f;
  projection.m[9] = 0.0f;
  projection.m[10] = (near_ + far_) / d;
  projection.m[11] = (2 * near_ * far_) / d;
...
void Camera::ClipOblique(const Vector3& pos, const Vector3& normal)
{
  ...
  const Vector4 q =
    projection.Inverse() * Vector4((cplane.x < 0.0f ? 1.0f : -1.0f), (cplane.y < 0.0f ? 1.0f : -1.0f), 1.0f, 1.0f);
